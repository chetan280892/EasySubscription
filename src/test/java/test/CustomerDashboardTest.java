...
public class CustomerDashboardTest extends BaseTest {
	...
	public void openContract() {
		...
		customerDashboard.skipButtonText(driver);
	...
	@Test(priority = 7)
	public void verifyDateAfterSkip() {
		CustomerDashboard customerDashboard = new CustomerDashboard(driver);
		// Click on the "Account" button to open login Page
		customerDashboard.clickAccount();
		// Enter email for login
	...
	@Test(priority = 8)
	public void verifyRescheduleNextOrder() {
		CustomerDashboard customerDashboard = new CustomerDashboard(driver);
		// Click on the "Account" button to open login Page
		customerDashboard.clickAccount();
		// Enter email for login
		...
		// Click on the contract
		customerDashboard.clickContract(driver);
		customerDashboard.clickRescheduleButton(driver);
		customerDashboard.selectDate();
		customerDashboard.clickApply();
		String text = customerDashboard.orderRescheduleText(driver);
		Assert.assertEquals(text, "Order Rescheduled.");
	}
	/*
	 * @Test(priority = 9) public void verifyCustomCSS() { CustomerDashboard
	 * customerDashboard = new CustomerDashboard(driver);
	 * customerDashboard.clickAccount();
	 * customerDashboard.enterEmail("automationtesting@gmail.com");
	 * customerDashboard.enterPassword("123456789"); customerDashboard.clickSign();
	 * customerDashboard.clickManageButton();
	 * BaseTest.switchToWindow("ESA Customer Portal");
	 * customerDashboard.waitContract(driver); }
	 */
	@Test(priority = 10)
	public void verifyRescheduleNextOrderDate() throws InterruptedException {
		CustomerDashboard customerDashboard = new CustomerDashboard(driver);
		// Click on the "Account" button to open login Page
		customerDashboard.clickAccount();
		// Enter email for login
		...
		customerDashboard.selectDate();
		Thread.sleep(3000);
		String date = customerDashboard.getSelectDate();
		System.out.println(date);
		customerDashboard.clickApply();
		String nextOrderDate = customerDashboard.getNextOrderDate();
